...
<template>
  ...
  <Teleport to="body">
    <transition name="slide-fade">
      <div v-if="isOpen"
        class="fixed z-50 flex flex-col max-w-full bg-white dark:bg-gray-800 dark:text-gray-300 bg-clip-padding transition-all duration-300 ease-in-out h-full shadow"
        :class="[classPosition, position]" :style="{
          width: width,
          height: height,
          maxWidth: '100vw',
          maxHeight: '100vh',
        ...
        <slot name="header">
          <div class="flex justify-between p-4 border-b-2 dark:border-gray-700 border-b-gray-200 items-center">
    ...
    <transition enter-active-class="ease-out duration-300" enter-from-class="opacity-0" enter-to-class="opacity-100"
      ...
      <slot name="backdrop" :isOpen="isOpen" :closeOffCanvas="closeOffCanvas">
        <div v-if="isOpen" class="fixed inset-0 transform transition-all z-10">
          <div class="absolute inset-0 bg-gray-900 opacity-50" @click="
            closeOffCanvas();
          emit('backdrop-click');">
          </div>
        </div>
